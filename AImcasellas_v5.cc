 ...
 */
#define PLAYER_NAME mcasellas_v5
...
struct PLAYER_NAME : public Player {
    ...
    void dijkstra(int u, int v, stack<int>& result) {
        ...
        while (!pQ.empty() && pQ.top().second != v) {
            ...
            pQ.pop();
            if (!visitat[intermig]) {
                visitat[intermig] = true;
                for (int i = 0; i < G[intermig].size(); ++i) {
                    pair<int, int> adjacent = G[intermig][i];
                    if (cell((adjacent.second)/cols(),(adjacent.second)%cols()).type != WATER and distance[intermig] + adjacent.first < distance[adjacent.second]) {
    ...
    void calc_direc(Pos act, int id) {
        ...
        for (map<int,vector<Pos>>::const_iterator it = camins.begin(); it != camins.end(); it++) {
            for (int i = 0; i < (it->second).size(); i++){
                if (path_owner(it->first) != me() and distancia(act, it->second[i]) < distancia(act, proper_cami) and act != it->second[i])
                    proper_cami = it->second[i];
            }
        }
        
        for (map<int,vector<Pos>>::const_iterator it = ciutats.begin(); it != ciutats.end(); it++) {
            for (int i = 0; i < (it->second).size(); i++){
                if (city_owner(it->first) != me() and it->second[i] != act and distancia(act, it->second[i]) < distancia(act, propera_ciutat) and act != it->second[i])
                    propera_ciutat = it->second[i];
            }
        }
        Pos propera {numeric_limits<int>::max(),numeric_limits<int>::max()};
        if (propera_ciutat == Pos(numeric_limits<int>::max(),numeric_limits<int>::max())) propera = proper_cami;
        
        else propera = propera_ciutat;
